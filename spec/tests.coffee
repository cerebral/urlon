...
beforeEach ->
  jasmine.addMatchers
    toBeWorking: (util, customEqualityTesters) ->
      compare: (obj) ->
        str1 = URLON.stringify obj
        parsed = URLON.parse str1
        str2 = URLON.stringify parsed

        str3 = JSON.stringify obj
        str4 = JSON.stringify parsed

        {
          pass: str1 is str2 && str3 is str4
          message: "URLON original: #{str1}\nURLON parse: #{str2}\nJSON original: #{str3}\nJSON parsed: #{str4}"
        }
...
describe "URLON", ->
  ...
  describe "undefined", ->
    ...
    it 'throws to parse', ->
      expect(-> URLON.parse undefined).toThrow()
